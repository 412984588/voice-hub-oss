...
export class VoiceHubServer {
  ...
  private setupRoutes(): void {
    ...
    // 语音控制
    this.server.post(
      "/api/sessions/:sessionId/listening",
      async (request, reply) => {
        const { sessionId } = request.params as { sessionId: string };
    ...
    this.server.post(`${this.config.webhookPath}`, async (request, reply) => {
      ...
      const signature = this.getSingleHeaderValue(
        request.headers["x-webhook-signature"],
      );
      const timestamp = this.getSingleHeaderValue(
      ...
      if (signature && timestamp) {
        ...
        if (!this.isLegacyWebhookHeaderEnabled()) {
          return reply
            .code(401)
            .send({ error: "Legacy webhook header disabled" });
        }

        if (
          !legacySecret ||
          !this.constantTimeEquals(legacySecret, this.config.webhookSecret)
        ) {
          return reply.code(401).send({ error: "Unauthorized" });
        }
      }
  ...
  private extractAnnouncementText(payload: WebhookPayload): string | null {
    ...
    if (
      typeof normalized.announcementText === "string" &&
      normalized.announcementText.trim().length > 0
    ) {
  ...
  private async synthesizeText(
    text: string,
  ): Promise<{ frame: AudioFrame; source: "tts-api" | "fallback" }> {
  ...
  private isAuthorizedRequestHeaders(
    ...
    const apiKeyHeader = this.getSingleHeaderValue(
      headers["x-api-key"] as string | string[] | undefined,
    );
    if (
      apiKeyHeader &&
      this.constantTimeEquals(requiredKey, apiKeyHeader.trim())
    ) {
      return true;
    }
